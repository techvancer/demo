...
export default function Exams() {
    ...
    const fetchExams = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const [stuExams, examTbl, clTbl, secRows, subTbl, answers, questionRows] = await Promise.all([
                ...
                rest('questions_exams_employee_subjects_sections_tbl', {
                    ...
                    select: 'examid,classid,sectionid,subjectid,question_marks,status',
                }).catch(() => []),
            ]);
            ...
            // Build totalMark map per exam-class-section-subject
            const totalMarkMap = {};
            (questionRows || []).forEach(q => {
                const k = `${q.examid}-${q.classid}-${q.sectionid}-${q.subjectid}`;
                totalMarkMap[k] = (totalMarkMap[k] || 0) + (parseFloat(q.question_marks) || 0);
            });

            // Build set of exam keys that exist in questions_exams table
            const qExamKeys = new Set(
                (questionRows || []).map(q => `${q.examid}-${q.classid}-${q.sectionid}-${q.subjectid}`)
            );

            const seen = new Set();
            const rows = stuExams.filter(r => {
                const k = `${r.examid}-${r.classid}-${r.sectionid}-${r.subjectid}`;
                // Only include exams that have question definitions in questions_exams table
                if (!qExamKeys.has(k)) return false;
                ...
                // Respect DB-stored status (cancelled / submitted), else compute the simplified status model
                const dbStatus = String(exam?.status || '').toLowerCase();
                // Also check questions_exams table status (cancelled is stored there)
                const qExamRow = (questionRows || []).find(q =>
                ...
                let status;
                if (qStatus === 'cancelled' || dbStatus === 'cancelled') status = 'cancelled';
                else if (qStatus === 'submitted' || dbStatus === 'submitted') status = 'submitted';
                else if (['marked', 'completed', 'inprogress'].includes(qStatus) || ['marked', 'completed', 'inprogress'].includes(dbStatus) || marksEntered > 0) status = 'marked';
                else status = 'new';
                const totalMark = totalMarkMap[key] || 0;
                return {
    ...
    const handleCancel = async () => {
        ...
        setCancelling(true);
        try {
            const params = `examid=eq.${e.examid}&employeeid=eq.${user.employeeid}&schoolid=eq.${user.schoolid}&branchid=eq.${user.branchid}`;
            const res = await fetch(`${BASE}/questions_exams_employee_subjects_sections_tbl?${params}`, {
                method: 'PATCH',
                headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json', Prefer: 'return=minimal' },
                body: JSON.stringify({ status: 'cancelled' }),
            });
            if (!res.ok) throw new Error('Failed to cancel exam');
                            ...
                            <tbody className="divide-y divide-[#e2e8f0]">
                                {filteredExams.map((e, i) => (
                                    <motion.tr key={`${e.examid}-${e.classid}-${e.sectionid}`}
                                        ...
                                        className={`transition-colors cursor-pointer ${e.status === 'cancelled' ? 'opacity-50 bg-slate-50 line-through-cells' : 'hover:bg-slate-50'}`}
                                        onClick={() => e.status !== 'cancelled' && navigate(`/exams/${e.examid}/${e.classid}/${e.sectionid}/${e.subjectid}/upload`)}>
                                        <td className={`py-4 px-6 text-sm font-bold ${e.status === 'cancelled' ? 'text-[#94a3b8] line-through' : 'text-[#0f172a]'}`}>{e.classname}</td>
                                        <td className="py-4 px-6">
                                            <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded text-xs font-bold border ${e.status === 'cancelled' ? 'bg-slate-100 text-slate-400 border-slate-200 line-through' : 'bg-[#eff6ff] text-[#1d4ed8] border-blue-100'}`}>{e.sectionname}</span>
                                        </td>
                                        <td className={`py-4 px-6 text-sm ${e.status === 'cancelled' ? 'text-[#94a3b8] line-through' : 'text-[#475569]'}`}>{e.subjectname}</td>
                                        <td className={`py-4 px-6 text-sm font-semibold ${e.status === 'cancelled' ? 'text-[#94a3b8] line-through' : 'text-[#0f172a]'}`}>{e.examname}</td>
                                        <td className="py-4 px-6">
                                                    ...
                                                    <button title="Process Marks"
                                                        onClick={async (ev) => {
                                                            ...
                                                            try {
                                                                ...
                                                                const submitRes = await fetch(`${BASE}/questions_exams_employee_subjects_sections_tbl?examid=eq.${e.examid}&employeeid=eq.${user.employeeid}&classid=eq.${e.classid}&sectionid=eq.${e.sectionid}&subjectid=eq.${e.subjectid}&schoolid=eq.${user.schoolid}&branchid=eq.${user.branchid}`, {
                                                                    method: 'PATCH',
                                                                    headers: {
                                                                ...
                                                                if (!submitRes.ok) {
                                                                    ...
                                                                    throw new Error(`Status update failed (${submitRes.status}): ${errBody}`);
                                                                }
                                                                if (response.ok) {
                                            ...
                                            <div style={{ height: 280 }}>
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <ComposedChart data={bellData} margin={{ top: 10, right: 30, left: 0, bottom: 30 }} barCategoryGap="10%">
                                                        ...
                                                        <Tooltip
                                                            contentStyle={{ borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: 12, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}
                                                            formatter={(value, name) => [name === 'count' ? `${value} ${t('students', lang)}` : value.toFixed(2), name === 'count' ? t('markDistributionDetail', lang) : isAr ? 'المنحنى الطبيعي' : 'Normal Curve']}
                                                        />
                                                        <Legend verticalAlign="top" align="right" iconType="circle" formatter={v => <span style={{ fontSize: 11, color: '#475569', fontWeight: 600 }}>{v === 'count' ? t('markDistributionDetail', lang) : isAr ? 'المنحنى الطبيعي' : 'Normal Curve'}</span>} />
