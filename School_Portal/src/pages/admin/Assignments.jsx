...
export default function AdminAssignments() {
    ...
    const assignSubjectToClass = async (subjectid, classid, sectionid, stageid) => {
        ...
        // If already exists, it's fine — subject is already linked to this class-section
                ...
                <div className="overflow-x-auto" dir={isAr ? 'rtl' : 'ltr'}>
                    <table className={`w-full ${isAr ? 'text-right' : 'text-left'} border-collapse`}>
                        <thead className="bg-[#f8fafc] border-b border-[#e2e8f0]">
                            <tr>
                                {activeTab === 'students' && <><SortableTh col="studentname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['studentname']} isSearchOpen={activeSearch==='studentname'} onSearchOpen={()=>setActiveSearch('studentname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('studentname','');}} onSearchChange={v=>setColumnSearch('studentname',v)}>{t('student', lang)}</SortableTh><SortableTh col="stagename" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['stagename']} isSearchOpen={activeSearch==='stagename'} onSearchOpen={()=>setActiveSearch('stagename')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('stagename','');}} onSearchChange={v=>setColumnSearch('stagename',v)}>{t('stage', lang)}</SortableTh><SortableTh col="classname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['classname']} isSearchOpen={activeSearch==='classname'} onSearchOpen={()=>setActiveSearch('classname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('classname','');}} onSearchChange={v=>setColumnSearch('classname',v)}>{t('class', lang)}</SortableTh><SortableTh col="sectionname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['sectionname']} isSearchOpen={activeSearch==='sectionname'} onSearchOpen={()=>setActiveSearch('sectionname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('sectionname','');}} onSearchChange={v=>setColumnSearch('sectionname',v)}>{t('section', lang)}</SortableTh><th className="px-6 py-4 text-[11px] font-black text-[#64748b] uppercase">{t('actions', lang)}</th></>}
                                {activeTab === 'subjects' && <><SortableTh col="subjectname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['subjectname']} isSearchOpen={activeSearch==='subjectname'} onSearchOpen={()=>setActiveSearch('subjectname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('subjectname','');}} onSearchChange={v=>setColumnSearch('subjectname',v)}>{t('subject', lang)}</SortableTh><SortableTh col="classname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['classname']} isSearchOpen={activeSearch==='classname'} onSearchOpen={()=>setActiveSearch('classname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('classname','');}} onSearchChange={v=>setColumnSearch('classname',v)}>{t('class', lang)}</SortableTh><SortableTh col="sectionname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['sectionname']} isSearchOpen={activeSearch==='sectionname'} onSearchOpen={()=>setActiveSearch('sectionname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('sectionname','');}} onSearchChange={v=>setColumnSearch('sectionname',v)}>{t('section', lang)}</SortableTh><th className="px-6 py-4 text-[11px] font-black text-[#64748b] uppercase">{t('actions', lang)}</th></>}
                                {activeTab === 'supervisors' && <><SortableTh col="supervisorname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['supervisorname']} isSearchOpen={activeSearch==='supervisorname'} onSearchOpen={()=>setActiveSearch('supervisorname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('supervisorname','');}} onSearchChange={v=>setColumnSearch('supervisorname',v)}>{t('supervisor', lang)}</SortableTh><SortableTh col="stagename" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['stagename']} isSearchOpen={activeSearch==='stagename'} onSearchOpen={()=>setActiveSearch('stagename')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('stagename','');}} onSearchChange={v=>setColumnSearch('stagename',v)}>{t('stage', lang)}</SortableTh><th className="px-6 py-4 text-[11px] font-black text-[#64748b] uppercase">{t('actions', lang)}</th></>}
                                {activeTab === 'teachers' && <><SortableTh col="teachername" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['teachername']} isSearchOpen={activeSearch==='teachername'} onSearchOpen={()=>setActiveSearch('teachername')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('teachername','');}} onSearchChange={v=>setColumnSearch('teachername',v)}>{t('teacher', lang)}</SortableTh><SortableTh col="subjectname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['subjectname']} isSearchOpen={activeSearch==='subjectname'} onSearchOpen={()=>setActiveSearch('subjectname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('subjectname','');}} onSearchChange={v=>setColumnSearch('subjectname',v)}>{t('subject', lang)}</SortableTh><SortableTh col="classname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['classname']} isSearchOpen={activeSearch==='classname'} onSearchOpen={()=>setActiveSearch('classname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('classname','');}} onSearchChange={v=>setColumnSearch('classname',v)}>{t('class', lang)}</SortableTh><SortableTh col="sectionname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} searchValue={columnSearch['sectionname']} isSearchOpen={activeSearch==='sectionname'} onSearchOpen={()=>setActiveSearch('sectionname')} onSearchClose={()=>{setActiveSearch(null);setColumnSearch('sectionname','');}} onSearchChange={v=>setColumnSearch('sectionname',v)}>{t('section', lang)}</SortableTh><th className="px-6 py-4 text-[11px] font-black text-[#64748b] uppercase">{t('actions', lang)}</th></>}
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                ...
                                <tr key={i} className="hover:bg-blue-50/20 transition-all">
                                    ...
                                    {activeTab === 'subjects' && <>
                                        ...
                                        {editingRow === i ? <>
                                            <td className="px-4 py-3 text-center"><select className={sel} value={editForm.classid || ''} onChange={e => setEditForm({...editForm, classid: e.target.value})}>{classes.map(c => <option key={c.classid} value={c.classid}>{t('class', lang)} {getField(c, 'classname', 'classname_en', lang) || c.classname}</option>)}</select></td>
                                    ...
                                    {activeTab === 'teachers' && <>
                                        ...
                                        <td className="px-4 py-3 text-center"><span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">{row.subjectname}</span></td>
                                        {editingRow === i ? <>
                                            <td className="px-4 py-3 text-center"><select className={sel} value={editForm.classid || ''} onChange={e => setEditForm({...editForm, classid: e.target.value})}>{classes.map(c => <option key={c.classid} value={c.classid}>{t('class', lang)} {getField(c, 'classname', 'classname_en', lang) || c.classname}</option>)}</select></td>
                                            <td className="px-4 py-3 text-center"><select className={sel} value={editForm.sectionid || ''} onChange={e => setEditForm({...editForm, sectionid: e.target.value})}>{sections.map(s => <option key={s.sectionid} value={s.sectionid}>{getField(s, 'sectionname', 'sectionname_en', lang) || s.sectionname}</option>)}</select></td>
                                            <td className="px-4 py-3 text-center"><div className="flex gap-2"><button onClick={handleSaveEdit} disabled={isLoading} className="btn-primary h-9 px-4 text-sm font-bold flex items-center gap-1 disabled:opacity-60">{isLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : null}{t('save', lang)}</button><button onClick={() => setEditingRow(null)} className="h-9 px-4 text-sm font-bold border border-[#e2e8f0] rounded-lg text-[#64748b] hover:bg-[#f8fafc]">{t('cancel', lang)}</button></div></td>
