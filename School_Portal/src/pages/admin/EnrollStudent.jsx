          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select className={sel} value={form.stageid} onChange={e => set('stageid', e.target.value)}>
                ...
                {stages.map((s, idx) => <option key={`stage-${s.stageid}-${idx}`} value={s.stageid}>{getField(s, 'stagename', 'stagename_en', lang)}</option>)}
              </select>
            </div>
            <div>
              ...
              <select className={sel} value={form.divisionid} onChange={e => set('divisionid', e.target.value)}>
                ...
                {divisions.map((d, idx) => <option key={`div-${d.divisionid}-${idx}`} value={d.divisionid}>{getField(d, 'divisionname', 'divisionname_en', lang) || d.divisionname}</option>)}
              </select>
            </div>
            <div>
              ...
              <select className={sel} value={form.curriculumid} onChange={e => set('curriculumid', e.target.value)}>
                <option value="">{t('default', lang)}</option>
                {curriculums.map((cur, idx) => <option key={`cur-${cur.curriculumid}-${idx}`} value={cur.curriculumid}>{getField(cur, 'curriculumname', 'curriculumname_en', lang) || cur.curriculumname}</option>)}
              </select>
            </div>
          </div>
        </div>
