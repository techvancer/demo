  ...
  const checkDuplicate = async (classid, sectionid, subjectid, examid) => {
    ...
    try {
      const res = await fetch(
        `${BASE}/questions_exams_employee_subjects_sections_tbl?examid=eq.${examid}&employeeid=eq.${user.employeeid}&classid=eq.${assignment.classid}&sectionid=eq.${assignment.sectionid}&subjectid=eq.${assignment.subjectid}&select=questionid&limit=1`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const q of formData.questions) {
        // Upsert into questions_tbl first to satisfy FK constraint fk_studentanswer_question
        // Use onConflict=ignore in case the row already exists
        await fetch(`${BASE}/questions_tbl?on_conflict=questionid,examid`, {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({ questionid: nextQid, questionname: `Q${nextQid}`, notes: null, examid: base.examid }),
        });
