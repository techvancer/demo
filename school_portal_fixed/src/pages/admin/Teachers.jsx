...
export default function AdminTeachers() {
  ...
  useEffect(() => {
        ...
        try {
            ...
            if (newEmployeeid) {
                await fetch(`${SUPABASE_URL}/rest/v1/employees_types_tbl`, {
                    ...
                    body: JSON.stringify({ employeeid: newEmployeeid, typeid: 1 }),
                });
                if (formData.employeemobile) {
                    await fetch(`${SUPABASE_URL}/rest/v1/employee_tbl?employeeid=eq.${newEmployeeid}`, {
                        method: 'PATCH',
                        headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json' },
                        body: JSON.stringify({ employeemobile: formData.employeemobile }),
                    });
                }
            }
    ...
    const handleDelete = async () => {
        try {
            await fetch(`${SUPABASE_URL}/rest/v1/employees_types_tbl?employeeid=eq.${selectedTeacher.employeeid}`, { method: 'DELETE', headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}` } });
            await remove('employee_tbl', selectedTeacher.employeeid, 'employeeid');
            setIsDeleteModalOpen(false);
            addToast(t('teacherDeleted', lang), 'error');
            fetchData();
        } catch (err) { addToast(err.message, 'error'); }
    };
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-[#f8fafc] border-b border-[#e2e8f0]">
                            <tr>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('id', lang)}</th>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('name', lang)} (AR / EN)</th>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('email', lang)}</th>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('mobile', lang)}</th>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('subject', lang)}</th>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('class', lang)}</th>
                                <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider whitespace-nowrap">{t('section', lang)}</th>
                            ...
                            {!hasApplied ? (
                                ...
                                selectedTeacher?.employeeid === teacher.employeeid && isEditModalOpen ? (
                                    <tr key={teacher.employeeid} className="bg-amber-50/50">
                                        <td className="px-4 py-3 text-sm font-bold">#{teacher.employeeid}</td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('arabicName', lang)} value={formData.employeename} onChange={e => setFormData(f => ({ ...f, employeename: e.target.value }))} /></td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('englishName', lang)} value={getField(formData, 'employeename', 'employeename_en', lang)} onChange={e => setFormData(f => ({ ...f, employeename_en: e.target.value }))} /></td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('email', lang)} value={formData.employeeemail} onChange={e => setFormData(f => ({ ...f, employeeemail: e.target.value }))} /></td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('mobile', lang)} value={formData.employeemobile} onChange={e => setFormData(f => ({ ...f, employeemobile: e.target.value }))} /></td>
                                        <td className="px-4 py-3 text-xs text-[#94a3b8]">{teacher.subjectName}</td>
                                        <td className="px-4 py-3 text-xs text-[#94a3b8]">{teacher.classname}</td>
                                        <td className="px-4 py-3 text-xs text-[#94a3b8]">{teacher.sectionname}</td>
                                        <td className="px-4 py-3"><div className="flex gap-2"><button onClick={e => { e.preventDefault(); handleUpdate(e); }} className="px-3 py-2 rounded-lg bg-[#1d4ed8] text-white text-xs font-bold">{t('save', lang)}</button><button onClick={() => { setSelectedTeacher(null); setIsEditModalOpen(false); }} className="px-3 py-2 rounded-lg border text-xs font-bold">{t('cancel', lang)}</button></div></td>
                                    </tr>
                                ) : (
                                <tr key={teacher.employeeid} className="hover:bg-[#f8fafc] transition-colors">
                                    <td className="px-4 py-4"><span className="text-xs font-mono font-bold text-[#94a3b8] bg-slate-100 px-2 py-1 rounded">#{teacher.employeeid}</span></td>
                                    ...
                                    <td className="px-4 py-4 text-sm text-[#475569]">{teacher.employeeemail || '—'}</td>
                                    <td className="px-4 py-4 text-sm text-[#475569]">{teacher.employeemobile || '—'}</td>
                                    <td className="px-4 py-4"><span className="text-xs font-bold text-blue-600">{teacher.subjectName}</span></td>
                                    <td className="px-4 py-4 text-sm font-medium text-[#0f172a]">{teacher.classname !== '—' ? `${t('class', lang)} ${teacher.classname}` : '—'}</td>
                                    <td className="px-4 py-4">{teacher.sectionname !== '—' ? (<span className="inline-flex items-center justify-center px-2 py-0.5 rounded bg-[#eff6ff] text-[#1d4ed8] text-xs font-bold border border-blue-100 min-w-[28px]">{teacher.sectionname}</span>) : <span className="text-[#94a3b8] text-xs">—</span>}</td>
                                    <td className="px-4 py-4">
                            ...
                            <form onSubmit={handleCreate}>
                                ...
                                <div className="p-8 space-y-5">
                                    <div className="space-y-2">
                                        ...
                                        <input required type="text" className="input-field h-11" placeholder="e.g. أحمد محمود" value={formData.employeename} onChange={e => setFormData(f => ({ ...f, employeename: e.target.value }))} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-sm font-bold text-[#0f172a]">{t('englishNameLabel', lang)}</label>
                                        <input required type="text" className="input-field h-11" placeholder="e.g. Ahmed Mahmoud" value={getField(formData, 'employeename', 'employeename_en', lang)} onChange={e => setFormData(f => ({ ...f, employeename_en: e.target.value }))} />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input required type="email" className="input-field pl-10 h-11" placeholder="teacher@school.edu" value={formData.employeeemail} onChange={e => setFormData(f => ({ ...f, employeeemail: e.target.value }))} />
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="text" className="input-field pl-10 h-11" placeholder="e.g. 0791234567" value={formData.employeemobile} onChange={e => setFormData(f => ({ ...f, employeemobile: e.target.value }))} />
                                        </div>
                                    </div>
