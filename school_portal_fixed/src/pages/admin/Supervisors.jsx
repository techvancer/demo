...
export default function AdminSupervisors() {
        ...
        try {
            ...
            if (newId) {
                await fetch(`${SUPABASE_URL}/rest/v1/employees_types_tbl`, {
                    ...
                    body: JSON.stringify({ employeeid: newId, typeid: 2 }),
                });
    ...
    const handleDelete = async () => {
        try {
            await fetch(`${SUPABASE_URL}/rest/v1/employees_types_tbl?employeeid=eq.${selectedSup.employeeid}`, { method: 'DELETE', headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}` } });
            ...
            for (const row of validRows) {
                ...
                if (res.ok) {
                    ...
                    if (newId) {
                        ...
                        if (row.empmobile) await fetch(`${SUPABASE_URL}/rest/v1/employee_tbl?employeeid=eq.${newId}`, { method: 'PATCH', headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ employeemobile: row.empmobile }) });
                    }
                    count++;
                }
            }
            ...
            {csvErrors.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl overflow-hidden">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-red-100"><tr><th className="px-4 py-2 text-xs font-bold text-red-700">Row</th><th className="px-4 py-2 text-xs font-bold text-red-700">Name</th><th className="px-4 py-2 text-xs font-bold text-red-700">Errors</th></tr></thead>
                        <tbody className="divide-y divide-red-100 bg-white">{csvErrors.map((e, i) => (<tr key={i}><td className="px-4 py-2 font-bold text-[#1d4ed8]">Row {e.rowNum}</td><td className="px-4 py-2 font-semibold">{e.name}</td><td className="px-4 py-2 text-red-600">{e.errors.join(' · ')}</td></tr>))}</tbody>
                    </table>
                </div>
            )}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-[#f8fafc] border-b border-[#e2e8f0]">
                            <tr>{[t('id', lang), t('name', lang), t('email', lang), t('mobile', lang), t('division', lang), t('curriculum', lang), t('assignedStage', lang), t('actions', lang)].map(h => (
                        ...
                        <tbody className="divide-y divide-[#e2e8f0]">
                            {showAddRow && (
                                <tr className="bg-blue-50/40">
                                    ...
                                    <td className="px-4 py-3"><div className="flex flex-col gap-1"><input className="input-field h-9" placeholder={t('arabicName', lang)} value={formData.employeename} onChange={e => setFormData(p => ({ ...p, employeename: e.target.value }))} /><input className="input-field h-9" placeholder={t('englishName', lang)} value={getField(formData, 'employeename', 'employeename_en', lang)} onChange={e => setFormData(p => ({ ...p, employeename_en: e.target.value }))} /></div></td>
                                    ...
                                    <td className="px-4 py-3 text-xs text-[#94a3b8] italic" colSpan={3}>{t('fromAssignments', lang)}</td>
                                    <td className="px-4 py-3"><div className="flex gap-2"><button onClick={handleCreate} className="px-3 py-2 rounded-lg bg-[#1d4ed8] text-white text-xs font-bold">{isLoading ? '...' : t('save', lang)}</button><button onClick={() => { setShowAddRow(false); setFormData(EMPTY_FORM); }} className="px-3 py-2 rounded-lg border text-xs font-bold">{t('cancel', lang)}</button></div></td>
                                </tr>
                            )}
                            {!hasApplied ? (
                                ...
                                selectedSup?.employeeid === s.employeeid && isEditOpen ? (
                                    <tr key={s.employeeid} className="bg-amber-50/50">
                                        <td className="px-4 py-3 text-sm font-bold">#{s.employeeid}</td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('englishName', lang)} value={getField(editData, 'employeename', 'employeename_en', lang)} onChange={e => setEditData(p => ({ ...p, employeename_en: e.target.value }))} /></td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('email', lang)} value={editData.employeeemail} onChange={e => setEditData(p => ({ ...p, employeeemail: e.target.value }))} /></td>
                                        <td className="px-4 py-3"><input className="input-field h-9 w-full" placeholder={t('mobile', lang)} value={editData.employeemobile} onChange={e => setEditData(p => ({ ...p, employeemobile: e.target.value }))} /></td>
                                        <td className="px-4 py-3 text-xs text-[#94a3b8]">{s.divisionname}</td>
                                        <td className="px-4 py-3 text-xs text-[#94a3b8]">{s.curriculumname}</td>
                                        <td className="px-4 py-3 text-xs text-[#94a3b8]">{s.stageName}</td>
                                        <td className="px-4 py-3"><div className="flex gap-2"><button onClick={handleUpdate} className="px-3 py-2 rounded-lg bg-[#1d4ed8] text-white text-xs font-bold">{t('save', lang)}</button><button onClick={() => { setSelectedSup(null); setIsEditOpen(false); }} className="px-3 py-2 rounded-lg border text-xs font-bold">{t('cancel', lang)}</button></div></td>
                                    </tr>
                                ) : (
                                    <tr key={s.employeeid} className="hover:bg-[#f8fafc] transition-colors">
                                        <td className="px-4 py-4"><span className="text-xs font-mono font-bold text-[#94a3b8] bg-slate-100 px-2 py-1 rounded">#{s.employeeid}</span></td>
                                        <td className="px-4 py-4 text-sm font-bold text-[#0f172a]">{getField(s, 'employeename', 'employeename_en', lang) || s.employeename || '—'}</td>
                                        <td className="px-4 py-4 text-sm text-[#475569]">{s.employeeemail || '—'}</td>
                                        <td className="px-4 py-4 text-sm text-[#475569]">{s.employeemobile || '—'}</td>
                                        <td className="px-4 py-4 text-sm text-[#475569]">{s.divisionname}</td>
                                        <td className="px-4 py-4 text-sm text-[#475569]">{s.curriculumname}</td>
                                        <td className="px-4 py-4"><span className="text-[10px] font-bold text-teal-600 bg-teal-50 px-2 py-1 rounded-full uppercase">{s.stageName}</span></td>
