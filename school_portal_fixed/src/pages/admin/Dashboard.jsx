...
export default function AdminDashboard() {
    ...
    const fetchStats = useCallback(async (filters = {}) => {
        ...
        try {
            ...
            const filteredSupIds     = hasEmpFilter ? new Set((supAssignRows  || []).map(r => r.employeeid)) : null;
            const teacherTypes    = hasEmpFilter ? (allTeacherTypes    || []).filter(t => filteredTeacherIds.has(t.employeeid)) : allTeacherTypes;
            const supervisorTypes = hasEmpFilter ? (allSupervisorTypes || []).filter(t => filteredSupIds.has(t.employeeid))     : allSupervisorTypes;
    ...
    const handleApply = (vals) => { setApplied(vals); setHasApplied(true); try { sessionStorage.setItem('adminDash_applied', JSON.stringify(vals)); sessionStorage.setItem('adminDash_hasApplied', 'true'); } catch {} fetchStats(vals); };
    const handleReset = () => { const r = { ...EMPTY_FILTER, employeeid: 'All' }; setApplied(r); setHasApplied(false); try { sessionStorage.removeItem('adminDash_applied'); sessionStorage.removeItem('adminDash_stats'); sessionStorage.removeItem('adminDash_gradeData'); sessionStorage.removeItem('adminDash_topClasses'); sessionStorage.setItem('adminDash_hasApplied', 'false'); } catch {} };
    ...
    const statCards = [
        { title: t('supervisors', lang),    value: stats.supervisors,       icon: Shield,        color: 'bg-rose-100 text-rose-600',     delay: 0,    path: () => ({ to: '/admin/supervisors', state: buildState(['stageid','curriculumid','divisionid']) }) },
        { title: t('teachers', lang),       value: stats.teachers,          icon: Users,         color: 'bg-blue-100 text-blue-600',     delay: 0.05, path: () => ({ to: '/admin/teachers', state: buildState(['curriculumid','divisionid','stageid','classid','sectionid','subjectid','employeeid']) }) },
        ...
        { title: t('exams', lang),          value: stats.examsThisSemester, icon: ClipboardList, color: 'bg-purple-100 text-purple-600', delay: 0.3,  path: () => ({ to: '/admin/exams', state: null }) },
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-12">
            <div>
                ...
                <p className="text-[#64748b] text-sm">{t('welcomeBack', lang)}, {user?.name}. {user?.schoolName && `${user.schoolName} · ${user.branchName}`}</p>
            </div>
