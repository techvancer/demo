    ...
    return (
        <div className="space-y-6 animate-fade-in pb-12">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-[#0f172a]">Define Questions</h1>
                    ...
                    <div className="card p-6 bg-[#0f172a] text-white rounded-2xl shadow-xl">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between border-b border-white/10 pb-3">
                                <span className="text-white/60 text-xs">Total Questions</span>
                                <span className="font-bold">{questions.length}</span>
                            </div>
                            <div className="flex justify-between border-b border-white/10 pb-3">
                                <span className="text-white/60 text-xs">Total Marks</span>
                ...
                {isAddModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl overflow-hidden p-8">
                            <h2 className="text-xl font-bold text-[#0f172a] mb-6">New Question</h2>
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-[#64748b] uppercase tracking-wider">Question Text</label>
                                    <textarea rows={3} className="input-field py-3" placeholder="Enter question description..." value={newQ.text} onChange={e => setNewQ({ ...newQ, text: e.target.value })} />
                                </div>
                            ...
                            <div className="flex gap-3 mt-8">
                                <button onClick={() => setIsAddModalOpen(false)} className="flex-1 font-bold text-[#64748b]">Discard</button>
                                <button onClick={handleAdd} className="flex-1 btn-primary !bg-black !text-white h-12 shadow-lg shadow-slate-200">Confirm Question</button>
                            </div>
                        </motion.div>
                    </div>
                )}
