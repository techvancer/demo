...
import { insert, update, rest, SUPABASE_URL, SUPABASE_SERVICE_KEY } from '../lib/supabaseClient';
...
export default function EditMarks() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const assignments = empSec.filter(r => {
                ...
                return {
                    ...r,
                    classname:   getField(cl, 'classname', 'classname_en', lang)   || getField(cl, 'classname', 'classname_en', lang) || cl?.classname   || String(r.classid),
                    sectionname: getField(sec, 'sectionname', 'sectionname_en', lang) || sec?.sectionname || String(r.sectionid),
                    subjectname: getField(sub, 'subjectname', 'Subjectname_en', lang) || getField(sub, 'subjectname', 'Subjectname_en', lang) || sub?.subjectname || String(r.subjectid),
                };
            });
    ...
    }, [selSubject, selClass, selSection, user, allExams]);
                ...
                for (const q of questions) {
                    ...
                    try {
                        await insert('studentanswers_tbl', {
                            ...
                            semisterid: q.semisterid ?? 1, yearid: q.yearid ?? 2026, stageid: q.stageid ?? sec.stageid,
                            curriculumid: q.curriculumid ?? user.curriculumid ?? 1, divisionid: q.divisionid ?? user.divisionid ?? 1,
                        ...
                        if (insertError.message?.includes('duplicate')) {
                            try {
                                await fetch(`${BASE}/studentanswers_tbl?questionid=eq.${q.questionid}&studentid=eq.${stu.studentid}&examid=eq.${parseInt(selExam)}&employeeid=eq.${user.employeeid}&classid=eq.${sec.classid}&sectionid=eq.${sec.sectionid}&subjectid=eq.${sec.subjectid}`, {
                                    method: 'PATCH',
                                    headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json', Prefer: 'return=minimal' },
                                    body: JSON.stringify({ studentmark: String(numVal) }),
                                });
                                saved++;
    ...
    return (
        <div className="space-y-6 pb-12">
            <div>
                ...
                <p className="text-[#64748b] text-sm">{t('editMarksDesc', lang) || 'Select a class, section, subject and exam to edit student marks.'}</p>
            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50 border-b border-[#e2e8f0]">
                                            <th className="py-4 px-4 font-bold text-[#64748b] text-sm sticky left-0 bg-slate-50 z-10 min-w-[220px] cursor-pointer select-none" onClick={() => handleSort('fullName')}>{t('student', lang)}<span className="text-[#1d4ed8] ml-1">{sortArrow('fullName')}</span></th>
