...
export default function SupervisorExams() {
    ...
    const fetchData = useCallback(async (filters = {}) => {
        ...
        try {
            ...
            const classStages = await rest('classes_stages_tbl', { ...stageParam, select: 'classid,stageid' });
            ...
            if (supervisedClassIds.length === 0) { setExams([]); return; }
            ...
            const [examRows, examTypes, classRows, sectionRows, subjectRows, empList, answerRows, questionRows] = await Promise.all([
                ...
                rest('employee_tbl', { schoolid: `eq.${sid}`, branchid: `eq.${bid}`, select: '*' }),
                rest('studentanswers_tbl', { schoolid: `eq.${sid}`, branchid: `eq.${bid}`, classid: `in.(${supervisedClassIds})`, select: 'examid,classid,sectionid,subjectid,employeeid,studentid,studentmark,questionid' }),
                rest('questions_exams_employee_subjects_sections_tbl', { schoolid: `eq.${sid}`, branchid: `eq.${bid}`, classid: `in.(${supervisedClassIds})`, select: 'examid,classid,sectionid,subjectid,employeeid,questionid,question_marks' }),
            ]);
    ...
    const filterData = useMemo(() => {
        ...
        return {
            ...baseFilterData,
            exams:    fromExams(e => e.examid,    e => e.examid,    e => e.examName,    'allExams'),
            subjects: fromExams(e => e.subjectid, e => e.subjectid, e => e.subjectName, 'allSubjects'),
            employees: fromExams(e => e.employeeid, e => e.employeeid, e => e.teacherName, 'allEmployees'),
        };
        ...
        <div className="space-y-6 animate-fade-in pb-12">
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
                <div className="px-6 py-4 border-b border-[#e2e8f0] flex items-center gap-2">
                    ...
                    <span className="ml-auto text-xs text-[#94a3b8]">{filtered.length} {t('exams', lang)}</span>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-[#f8fafc] border-b border-[#e2e8f0]">
                            <tr>
  <SortableTh col="classname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('class', lang)}</SortableTh>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('section', lang)}</th>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('subject', lang)}</th>
  <SortableTh col="examName" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('exam', lang)}</SortableTh>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('teacher', lang)}</th>
  <SortableTh col="avgMarks" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('avgMarks', lang)}</SortableTh>
  <SortableTh col="passedCount" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('passed', lang)}</SortableTh>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('took', lang)}</th>
</tr>
                        </thead>
                        <tbody className="divide-y divide-[#e2e8f0]">
                            {!hasApplied ? <tr><td colSpan={8} className="px-6 py-12 text-center text-[#94a3b8] font-medium">Press <span className="text-[#1d4ed8] font-bold">Apply Filter</span> to load data.</td></tr>
                            : loading ? <tr><td colSpan={8} className="px-6 py-12 text-center text-[#94a3b8]">{t('loading', lang)}</td></tr>
                            : filtered.length === 0 ? <tr><td colSpan={8} className="px-6 py-12 text-center text-[#94a3b8]">{t('noData', lang)}</td></tr>
                            : filtered.map((e, i) => (
                                <tr key={i} className="hover:bg-[#f8fafc]">
                                    <td className="px-4 py-4 text-sm font-bold text-[#0f172a]">{e.classname}</td>
                                    <td className="px-4 py-4"><span className="inline-flex items-center justify-center px-2 py-0.5 rounded bg-[#eff6ff] text-[#1d4ed8] text-xs font-bold border border-blue-100 min-w-[28px]">{e.sectionname}</span></td>
                                    <td className="px-4 py-4 text-sm text-[#475569]">{e.subjectName}</td>
                                    <td className="px-4 py-4">
                                        <p className="text-sm font-bold text-[#0f172a]">{e.examName}</p>
                                    </td>
                                    <td className="px-4 py-4 text-sm text-[#475569]">{e.teacherName}</td>
                                    <td className="px-4 py-4 text-sm font-bold text-[#0f172a]">{e.avgMarks == null ? '—' : e.avgMarks.toFixed(1)}</td>
                                    <td className="px-4 py-4 text-sm font-bold text-green-700">{e.passedCount}</td>
                                    <td className="px-4 py-4 text-sm font-bold text-[#0f172a]">{e.tookCount}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
