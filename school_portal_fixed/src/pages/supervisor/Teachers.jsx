...
export default function SupervisorTeachers() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [supStages, classStages, empTypes, empList, assignments, classRows, sectionRows, subjectRows, answerRows] = await Promise.all([
                rest('employees_types_stages_tbl', { employeeid: `eq.${user.employeeid}`, schoolid: `eq.${user.schoolid}`, branchid: `eq.${user.branchid}`, select: '*' }),
                rest('classes_stages_tbl', { select: '*' }),
                ...
                rest('studentanswers_tbl', { schoolid: `eq.${user.schoolid}`, branchid: `eq.${user.branchid}`, select: 'employeeid,studentmark,classid,sectionid,subjectid' }),
            ]);
            ...
            // Build map of employeeid → all assignments for filter matching
            const aMap = {};
            ...
            const result = supervisedTeacherIds.map(employeeid => {
                ...
                const subjects = [...new Set(assigns.map(a => {
                    ...
                    return getField(sub, 'subjectname', 'Subjectname_en', lang) || sub?.subjectname_en || getField(sub, 'subjectname', 'Subjectname_en', lang) || sub?.subjectname || `Subject ${a.subjectid}`;
                }))];
        ...
        <div className="space-y-6 animate-fade-in pb-12">
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-4 flex items-center gap-3">
                <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder={t('searchTeachersPlaceholder', lang)} className="input-field pl-10 h-10 w-full" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
            </div>
  ...
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('email', lang)}</th>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('subjects', lang)}</th>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('classes', lang)}</th>
  <SortableTh col="avgMarks" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('avgMarks', lang)}</SortableTh>
                    ...
                    <tbody className="divide-y divide-[#e2e8f0]">
                        {!hasApplied ? (
                                <tr><td colSpan={6} className="px-6 py-12 text-center text-[#94a3b8] font-medium">{t('pressApplyToLoad', lang)}</td></tr>
                            ) : loading ? (
                            <tr><td colSpan={6} className="px-6 py-12 text-center text-[#94a3b8]">{t('loading', lang)}</td></tr>
                        ) : filtered.length === 0 ? (
                            <tr><td colSpan={6} className="px-6 py-12 text-center text-[#94a3b8]">{t('noData', lang)}</td></tr>
                        ) : filtered.map(teacher => (
                            <tr key={teacher.employeeid} className="hover:bg-[#f8fafc]">
                                <td className="px-4 py-4"><span className="text-xs font-mono bg-slate-100 px-2 py-1 rounded text-[#94a3b8]">#{teacher.employeeid}</span></td>
                                <td className="px-4 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="h-9 w-9 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-xs uppercase">{(getField(teacher, 'employeename', 'employeename_en', lang) || teacher.employeename)?.split(' ').map(n=>n[0]).join('').slice(0,2)}</div>
                                        <span className="text-sm font-bold text-[#0f172a]">{getField(teacher, 'employeename', 'employeename_en', lang) || teacher.employeename}</span>
                                    </div>
                                </td>
                                <td className="px-4 py-4 text-sm text-[#475569]">{teacher.employeeemail || '—'}</td>
                                <td className="px-4 py-4">
                                    <div className="flex flex-wrap gap-1">
                                        {teacher.subjects.length > 0 ? teacher.subjects.map((s,i) => <span key={i} className="text-xs font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded">{s}</span>) : <span className="text-xs text-[#94a3b8]">{t('notAssigned', lang)}</span>}
                                    </div>
                                </td>
                                <td className="px-4 py-4 text-xs text-[#64748b]">{teacher.classLabels.join(', ') || '—'}</td>
                                <td className="px-4 py-4 text-sm font-bold text-[#0f172a]">{teacher.avgMarks == null ? '—' : teacher.avgMarks.toFixed(1)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
