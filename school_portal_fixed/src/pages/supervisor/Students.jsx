...
export default function SupervisorStudents() {
    ...
    const fetchData = useCallback(async (filters = {}) => {
        ...
        try {
            ...
            if (supervisedClassIds.length === 0) { setStudents([]); return; }
  ...
  <SortableTh col="fullname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('name', lang)}</SortableTh>
  <SortableTh col="classname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('class', lang)}</SortableTh>
  <SortableTh col="sectionname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('section', lang)}</SortableTh>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('stage', lang)}</th>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('parent', lang)}</th>
  <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase">{t('mobile', lang)}</th>
</tr>
                    </thead>
                    <tbody className="divide-y divide-[#e2e8f0]">
                        ...
                        : filtered.map(s => (
                            <tr key={s.studentid} className="hover:bg-[#f8fafc]">
                                <td className="px-4 py-4"><span className="text-xs font-mono bg-slate-100 px-2 py-1 rounded text-[#94a3b8]">#{s.studentid}</span></td>
                                <td className="px-4 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="h-8 w-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-xs">{(_getStudentName(s, lang) || '').split(' ').map(n=>n[0]).join('').slice(0,2)}</div>
                                ...
                                <td className="px-4 py-4"><span className="inline-flex items-center justify-center px-2 py-0.5 rounded bg-[#eff6ff] text-[#1d4ed8] text-xs font-bold border border-blue-100">{s.sectionname}</span></td>
                                <td className="px-4 py-4 text-xs text-[#64748b]">{s.stageName}</td>
            ...
            <p className="text-xs text-[#94a3b8]">{t('total', lang)}: {filtered.length} {t('student', lang)}</p>
        </div>
    );
}
