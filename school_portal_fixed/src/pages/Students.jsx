...
export default function Students() {
    ...
    useEffect(() => {
        ...
        const merged = Object.keys(fromUrl).length > 0 ? { ...EMPTY_FILTER, ...fromUrl } : { ...EMPTY_FILTER };
        if (Object.keys(fromUrl).length > 0) setDraft(merged);
        setHasApplied(true);
        fetchData(merged);
    }, [fetchData, user]);
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left min-w-[800px]">
                                <thead className="bg-[#f8fafc] border-b border-[#e2e8f0]">
                                    <tr>
                                        <SortableTh col="studentid" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">#</SortableTh>
                                        <SortableTh col="fullName" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="px-4">{t('studentFullName', lang) || 'Student Full Name'}</SortableTh>
                                        <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider">{t('email', lang)}</th>
                                        <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider">{t('mobile', lang)}</th>
                                        <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider">{t('classes', lang)}</th>
                                        <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider">{t('section', lang)}</th>
                                        <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider">{t('division', lang)}</th>
                                        <th className="px-4 py-4 text-xs font-bold text-[#64748b] uppercase tracking-wider">{t('curriculum', lang)}</th>
