...
import { insert, rest, SUPABASE_URL, SUPABASE_SERVICE_KEY } from '../lib/supabaseClient';
...
export default function UploadMarks() {
        ...
        try {
            const [examRows, classRows, sectionRows, subjectRows, questionRows, sectionStudentRows, studentRows, existingAnswers, assignmentRows] = await Promise.all([
                ...
                rest('questions_exams_employee_subjects_sections_tbl', {
                    ...
                    select: 'questionid,question_marks,semisterid,yearid,stageid,curriculumid,divisionid',
            ...
            setHasSubmitted(existingAnswers.length > 0);
    ...
    }, [addToast, classid, examid, sectionid, subjectid, user]);
    ...
    const handleMarkChange = (studentid, questionid, value) => {
        ...
        if (value !== '' && (isNaN(num) || num < 0)) {
            error = 'Must be 0 or above';
        } else if (value !== '' && num > maxMark) {
            error = `Max is ${maxMark}`;
        }
        ...
        for (const student of students) {
            for (const qObj of questions) {
                ...
                await insert('studentanswers_tbl', {
                    ...
                    sectionid: meta.sectionid,
                    classid: meta.classid,
                    semisterid: qObj.semisterid ?? meta.semisterid,
                    yearid: qObj.yearid ?? meta.yearid,
                    stageid: qObj.stageid ?? meta.stageid,
                    curriculumid: qObj.curriculumid ?? meta.curriculumid,
                    divisionid: qObj.divisionid ?? meta.divisionid,
                    branchid: user.branchid,
                    schoolid: user.schoolid,
                    typeid: 1,
        ...
        <div className="space-y-6 pb-12">
            ...
            <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200 p-6">
                    ...
                    <p className="text-xs text-blue-700 mt-2">Total in class</p>
                </div>

                {/* Questions Card */}
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border border-purple-200 p-6">
                    <p className="text-xs font-bold text-purple-600 uppercase tracking-wide">Questions</p>
                    <p className="text-3xl font-bold text-purple-900 mt-3">{questions.length}</p>
                    <p className="text-xs text-purple-700 mt-2">In this exam</p>
                </div>

                {/* Marks Entered Card */}
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl border border-green-200 p-6">
                    <p className="text-xs font-bold text-green-600 uppercase tracking-wide">Marks Entered</p>
                    <p className="text-3xl font-bold text-green-900 mt-3">
                        {Object.keys(marks).length}/{totalStudents}
                    </p>
                    <p className="text-xs text-green-700 mt-2">Progress</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl border border-emerald-200 p-6">
                    ...
                    <p className="text-3xl font-bold text-emerald-900 mt-3">Ready</p>
                    <p className="text-xs text-emerald-700 mt-2">Upload marks</p>
                </div>
            </div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="border-t border-[#e2e8f0] pt-6">
                        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
                            <h2 className="text-lg font-bold text-[#0f172a]">{t('manualEntry', lang) || 'Manual Entry'}</h2>
                        </div>
                        ...
                        {!students.length ? (
                            ...
                            <div className="py-12 text-center text-yellow-700 bg-yellow-50 rounded-xl border border-yellow-200">
                                {t('marksAlreadySubmitted', lang) || 'Marks for this exam have already been submitted.'}
                                <br />
                                {t('visitEditMarks', lang) || 'To modify them, please visit the Edit Marks page.'}
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <div className="overflow-x-auto border border-[#e2e8f0] rounded-xl">
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-50 border-b border-[#e2e8f0]">
                                            <tr>
                                                <th className="py-4 px-4 text-xs font-black text-[#64748b] uppercase tracking-wider">Student ID</th>
                                                    ...
                                                    {questions.map((q) => {
                                                        ...
                                                        <td key={q.questionid} className="py-3 px-4 text-center">
                                                            <input
                                                                ...
                                                                className={`w-20 h-10 text-center font-bold text-[#0f172a] border rounded-lg outline-none focus:ring-2 ${qErr ? 'border-red-400 bg-red-50 focus:ring-red-200' : 'border-[#dbe4f0] focus:border-[#1d4ed8] focus:ring-blue-100'}`}
                                                            />
