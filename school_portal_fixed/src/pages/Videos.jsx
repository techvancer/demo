...
export default function Videos() {
    ...
    useEffect(() => {
        ...
        (async () => {
            const [empSec, clTbl, secRows, subTbl, examTbl, semTbl, answers] = await Promise.all([
                ...
                rest('semisters_tbl',{ select: 'semisterid,semistername_en' }),
            ...
            const asgn = empSec.filter(r => {
                ...
                ...r,
                classname:   getClassName(clTbl.find(c => c.classid === r.classid), lang)   || String(r.classid),
                sectionname: getSectionName(secRows.find(s => s.sectionid === r.sectionid), lang) || String(r.sectionid),
                subjectname: getSubjectName(subTbl.find(s => s.subjectid === r.subjectid), lang) || String(r.subjectid),
            }));
            setAssignments(asgn);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const exOpts = unique.map(r => ({
                ...
                name: examsData.find(e => e.examid === r.examid)?.examname_en || `Exam ${r.examid}`,
            }));
    ...
    }, [selSubject, selSection, selClass, user, examsData, semestersData]);
    ...
    const handleSaveUrl = async (q) => {
        setSaving(true);
        try {
            await fetch(`${BASE}/questions_exams_employee_subjects_sections_tbl?questionid=eq.${q.questionid}&examid=eq.${q.examid}&employeeid=eq.${user.employeeid}`, {
                method: 'PATCH',
                headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json', Prefer: 'return=minimal' },
                body: JSON.stringify({ video_url: editUrl }),
            });
    ...
    const filterDefs = [
        { key: 'classid',    label: t('class', lang),  sel: selClass,    set: setSelClass,    opts: classOptions.map(a => ({ value: String(a.classid),   label: a.classname || '' })),   dep: false },
        { key: 'sectionid',  label: t('section', lang),  sel: selSection,  set: setSelSection,  opts: sectionOptions.map(a => ({ value: String(a.sectionid),  label: a.sectionname })), dep: !selClass },
        { key: 'subjectid',  label: t('subject', lang),  sel: selSubject,  set: setSelSubject,  opts: subjectOptions.map(a => ({ value: String(a.subjectid),  label: a.subjectname })), dep: !selSection },
        ...
        { key: 'semisterid', label: t('semester', lang), sel: selSemester, set: setSelSemester, opts: semesterOptions.map(s => ({ value: String(s.semisterid), label: t(s.semistername_en, lang) })), dep: !selSubject },
    ];
    ...
    return (
        <div className="space-y-6 pb-12">
            <div>
                ...
                <p className="text-[#64748b] text-sm">{t('manageVideosDesc', lang) || 'Manage YouTube videos attached to exam questions.'}</p>
            </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 border-b border-[#e2e8f0]">
                                <tr>{[t('questionid', lang) || 'Q#', t('class', lang), t('section', lang), t('subject', lang), t('exam', lang), t('status', lang), t('videoUrl', lang), t('actions', lang)].map(h => (
