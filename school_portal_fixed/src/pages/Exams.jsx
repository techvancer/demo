...
import { ClipboardList, Plus, Trash2, AlertTriangle, Loader2, Search, Upload, RotateCw, XCircle, Edit3, CheckCircle2 } from 'lucide-react';
...
import { rest, SUPABASE_URL, SUPABASE_SERVICE_KEY } from '../lib/supabaseClient';
...
async function deleteRows(table, filters) {
    ...
    return fetch(`${BASE}/${table}?${params}`, { method: 'DELETE', headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}` } });
}

// Status derived from whether studentmarks exist
function getStatus(examKey, answeredKeys) {
    if (answeredKeys.has(examKey)) return 'inprogress'; // Marks uploaded, ready for grading
    return 'new';
}
...
export default function Exams() {
    ...
    // Pre-fill applied filters from navigation state (e.g., from dashboard card click)
    const [applied, setApplied] = useState(() => {
        const incoming = location.state;
        if (incoming && typeof incoming === 'object') {
            return { ...EMPTY_FILTER, ...incoming };
        }
        return { ...EMPTY_FILTER };
    });
    ...
    const [teacherExamOptions, setTeacherExamOptions] = useState([]);
    ...
    const [colFilters, setColFilters] = useState({ classname: '', sectionname: '', subjectname: '', examname: '', status: '' });
    ...
    const fetchExams = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const [stuExams, examTbl, clTbl, secRows, subTbl, answers, allQuestions] = await Promise.all([
                rest('students_exams_employees_section_subjects_classes_semisters_cur', {
                    employeeid: `eq.${user.employeeid}`, select: 'examid,classid,sectionid,subjectid,stageid,semisterid,yearid,curriculumid,divisionid,studentid',
                }),
                ...
                rest('studentanswers_tbl', { employeeid: `eq.${user.employeeid}`, schoolid: `eq.${user.schoolid}`, branchid: `eq.${user.branchid}`, select: 'examid,classid,sectionid,subjectid,studentid' }),
                rest('questions_exams_employee_subjects_sections_tbl', { employeeid: `eq.${user.employeeid}`, select: 'examid,classid,sectionid,subjectid,question_marks' }),
            ]);
            // Build set of "answered" keys
            const aKeys = new Set(answers.map(a => `${a.examid}-${a.classid}-${a.sectionid}-${a.subjectid}`));
            setAnsweredKeys(aKeys);

            const seen = new Set();
            const rows = stuExams.filter(r => {
                const k = `${r.examid}-${r.classid}-${r.sectionid}-${r.subjectid}`;
                if (seen.has(k)) return false; seen.add(k); return true;
            }).map(r => {
                const exam = examTbl.find(e => e.examid === r.examid);
                ...
                const key = `${r.examid}-${r.classid}-${r.sectionid}-${r.subjectid}`;
                // Change 10: accurate marks entered count per exam-class-section-subject
                const examAnswers = answers.filter(a =>
                ...
                const marksEntered = new Set(examAnswers.map(a => a.studentid)).size;
                ...
                // Total Mark for this specific combo
                const examQs = (allQuestions || []).filter(q =>
                ...
                const totalMark = examQs.reduce((sum, q) => sum + (parseFloat(q.question_marks) || 0), 0);

                // Status logic:
                // New: 0 marks entered
                // Submitted: some marks entered but not completed
                // Completed: all students have marks
                // (Cancelled is manual)
                let status = marksEntered >= totalStudents && totalStudents > 0
                    ? 'completed'
                    : marksEntered > 0 ? 'submitted' : 'new';

                // Check for manual "cancelled" status if you have a way to track it.
                // For now, we'll stick to these 3 + manual cancelled.
                return {
                    ...r, status, marksEntered, totalStudents, totalMark,
            ...
            setTeacherExamOptions([{ value: 'All', label: t('allExams', lang) }, ...teacherExams.map(r => ({ value: String(r.examid), label: r.examname }))]);
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            await deleteRows('questions_exams_employee_subjects_sections_tbl', { examid: e.examid, employeeid: user.employeeid, classid: e.classid, sectionid: e.sectionid });
            await deleteRows('studentanswers_tbl', { examid: e.examid, employeeid: user.employeeid, classid: e.classid, sectionid: e.sectionid });
        ...
        (async () => {
            try {
                ...
                const allData = await Promise.all(examRows.map(async r => {
                    const [questionRows, sectionStudentRows, studentRows, answerRows] = await Promise.all([
                        rest('questions_exams_employee_subjects_sections_tbl', {
                            examid: `eq.${examId}`,
                            employeeid: `eq.${user.employeeid}`,
                        ...
                        rest('students_tbl', { select: 'studentid,studentfirstname_en,studentfathersname_en,studentsurname_en,studentfirstname_ar,studentfathersname_ar,studentsurname_ar,studentname' }),
                        rest('studentanswers_tbl', {
                            examid: `eq.${examId}`,
                            employeeid: `eq.${user.employeeid}`,
                            classid: `eq.${r.classid}`,
                    ...
                    const studentIds = (sectionStudentRows || []).map(s => s.studentid);

                    // Sum marks per student
                    const stuTotals = {};
                    studentIds.forEach(sid => { stuTotals[sid] = 0; });
                    (answerRows || []).forEach(a => {
                        if (stuTotals[a.studentid] !== undefined) {
                            stuTotals[a.studentid] += parseFloat(a.studentmark) || 0;
                        }
                    });
                ...
                const data = Object.entries(merged).map(([sid, earned]) => {
                    ...
                    const name = _getStudentName(stu, lang) || stu?.studentname || `#${sid}`;
    ...
    }, [applied.examid, exams, user]);

    const filteredExams = sortedExamsBase.filter(e => {
        ...
        const cfClass = !colFilters.classname || e.classname?.toLowerCase().includes(colFilters.classname.toLowerCase());
        const cfSec = !colFilters.sectionname || e.sectionname?.toLowerCase().includes(colFilters.sectionname.toLowerCase());
        const cfSub = !colFilters.subjectname || e.subjectname?.toLowerCase().includes(colFilters.subjectname.toLowerCase());
        const cfExam = !colFilters.examname || e.examname?.toLowerCase().includes(colFilters.examname.toLowerCase());
        const cfStatus = !colFilters.status || e.status?.toLowerCase().includes(colFilters.status.toLowerCase());
        return ms && mcur && mdiv && mc && msec && mex && msub && msem && cfClass && cfSec && cfSub && cfExam && cfStatus;
    });
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const results = await Promise.all(examRows.map(async e => {
                    const [answerRows, questionRows] = await Promise.all([
                        rest('studentanswers_tbl', {
                            employeeid: `eq.${user.employeeid}`,
                            examid: `eq.${e.examid}`,
                    ...
                    const totalMax = (questionRows || []).reduce((sum, q) => sum + (parseFloat(q.question_marks) || 0), 0);
    ...
    return (
        <div className="space-y-8 animate-fade-in relative pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <button onClick={() => navigate('/exams/create', { state: { filters: applied } })}
            ...
            <FilterBar
                filters={buildFilters(applied, { ...filterData, exams: teacherExamOptions })}
                onApply={vals => { setApplied(vals); setHasApplied(true); }}
            ...
            {filteredExams.length > 0 && (chartScores.length > 0 || chartLoading) && (() => {
                ...
                const maxMark = chartMaxMark;
                // Divide 0–maxMark into 10 equal bins
                const binSize = maxMark / 10;
                const bins = Array.from({ length: 10 }, (_, i) => ({
                    range: `${parseFloat((i * binSize).toFixed(1))}–${parseFloat(((i + 1) * binSize).toFixed(1))}`,
                    midpoint: parseFloat(((i + 0.5) * binSize).toFixed(1)),
                    count: 0,
                }));
                // scores are pct (0-100), convert back to raw mark for binning
                scores.forEach(s => {
                    const rawMark = (s / 100) * maxMark;
                    const idx = Math.min(Math.floor(rawMark / binSize), 9);
                    bins[idx].count += 1;
                });

                const total = scores.length;
                // Work in raw marks domain for correct bell curve
                const rawScores = scores.map(s => (s / 100) * maxMark);
                const mean = total > 0 ? rawScores.reduce((a, b) => a + b, 0) / total : maxMark / 2;
                const variance = total > 1
                    ? rawScores.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / (total - 1)
                    : Math.pow(maxMark / 6, 2);
                const std = Math.sqrt(variance) || (maxMark / 6);
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={histData} margin={{ top: 10, right: 30, left: 10, bottom: 35 }} barCategoryGap="8%">
                                    ...
                                    <XAxis
                                        ...
                                        label={{ value: 'Exam Scores (%)', position: 'insideBottom', offset: -22, fontSize: 12, fill: '#64748b', fontWeight: 600 }}
                                    />
                                    <YAxis
                                        allowDecimals={false}
                                        tick={{ fill: '#94a3b8', fontSize: 11 }}
                                        ...
                                        axisLine={false}
                                        width={40}
                                        label={{ value: 'Number of Students', angle: -90, position: 'insideLeft', offset: 5, fontSize: 12, fill: '#64748b', fontWeight: 600 }}
                                    />
                                    <Tooltip
                                        contentStyle={{ borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: 12, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}
                                        formatter={(value, name) => [
                                            name === 'count' ? `${value} student${value !== 1 ? 's' : ''}` : value.toFixed(2),
                                            name === 'count' ? 'Marks Distribution' : 'Normal Curve'
                                        ]}
                                    />
                                    <Legend
                                        verticalAlign="top"
                                        align="right"
                                        iconType="circle"
                                        formatter={v => <span style={{ fontSize: 11, color: '#475569', fontWeight: 600 }}>{v === 'count' ? 'Marks Distribution' : 'Normal Curve'}</span>}
                                    />
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 border-b border-[#e2e8f0]">
                                <tr>
                                    <SortableTh col="classname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} filterValue={colFilters.classname} onFilter={v => setColFilter('classname', v)}>{t('class', lang)}</SortableTh>
                                    ...
                                    <SortableTh col="examname" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} filterValue={colFilters.examname} onFilter={v => setColFilter('examname', v)}>{t('exam', lang)}</SortableTh>
                                    <SortableTh col="totalMark" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>{t('totalMark', lang) || 'Total Mark'}</SortableTh>
                                    <SortableTh col="marksEntered" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>{t('marksEntered', lang)}</SortableTh>
                                    <SortableTh col="status" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} filterValue={colFilters.status} onFilter={v => setColFilter('status', v)}>{t('status', lang)}</SortableTh>
                                    <th className="py-4 px-6 text-xs font-black text-[#64748b] uppercase tracking-wider text-center">{t('actions', lang)}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[#e2e8f0]">
                                {filteredExams.map((e, i) => (
                                    <motion.tr key={`${e.examid}-${e.classid}-${e.sectionid}`}
                                        ...
                                        <td className="py-4 px-6 text-sm font-bold text-[#0f172a]">{e.classname}</td>
                                        <td className="py-4 px-6">
                                            <span className="inline-flex items-center justify-center px-2 py-0.5 rounded bg-[#eff6ff] text-[#1d4ed8] text-xs font-bold border border-blue-100">{e.sectionname}</span>
                                        </td>
                                        <td className="py-4 px-6 text-sm text-[#475569]">{e.subjectname}</td>
                                        <td className="py-4 px-6 text-sm font-semibold text-[#0f172a]">{e.examname}</td>
                                        <td className="py-4 px-6">
                                            <span className="text-sm font-bold text-[#1d4ed8]">{e.totalMark ?? 0}</span>
                                        </td>
                                        <td className="py-4 px-6">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center justify-center gap-1.5">
                                                {/* Upload Marks (New/Submitted) */}
                                                {(e.status === 'new' || e.status === 'submitted') && (
                                                    <button
                                                        onClick={(event) => {
                                                            event.stopPropagation();
                                                            navigate(`/exams/${e.examid}/${e.classid}/${e.sectionid}/${e.subjectid}/upload-marks`);
                                                        }}
                                                        title={t('uploadMarks', lang)}
                                                        className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors border border-transparent hover:border-blue-100">
                                                        <Upload className="h-4 w-4" />
                                                    </button>
                                                )}

                                                {/* Edit Marks (Any state except Cancelled) */}
                                                {e.status !== 'cancelled' && e.status !== 'new' && (
                                                    <button
                                                        onClick={(event) => {
                                                            event.stopPropagation();
                                                            navigate(`/edit-marks/${e.examid}/${e.classid}/${e.sectionid}/${e.subjectid}`);
                                                        }}
                                                        title={t('editMarks', lang)}
                                                            ...
                                                            try {
                                                                const response = await fetch('https://n8n.srv1133195.hstgr.cloud/webhook-test/strat_grading', {
                                                                    ...
                                                                    body: JSON.stringify({
                                                                        ...
                                                                        examid: e.examid, classid: e.classid, sectionid: e.sectionid, subjectid: e.subjectid,
                                                                        employeeid: user.employeeid, timestamp: new Date().toISOString(),
                                                                    }),
                                                                });
                                                                if (response.ok) addToast('Process Marks workflow started!', 'success');
                                                                else addToast('Failed to start workflow', 'error');
                                                            } catch (err) { addToast(`Error: ${err.message}`, 'error'); }
                                                        }}
                                                        title={t('processMarks', lang) || 'Process Marks'}
                                                ...
                                                {e.status !== 'cancelled' && (
                                                    <button
                                                        onClick={async (event) => {
                                                            event.stopPropagation();
                                                            if (window.confirm(t('confirmCancel', lang) || 'Are you sure you want to cancel this exam?')) {
                                                                try {
                                                                    setExams(prev => prev.map(ex => (ex.examid === e.examid && ex.classid === e.classid && ex.sectionid === e.sectionid && ex.subjectid === e.subjectid) ? { ...ex, status: 'cancelled' } : ex));
                                                                    addToast('Exam cancelled!', 'info');
                                                                } catch (err) { addToast(err.message, 'error'); }
                                                            }
                                                        }}
                                                        title={t('cancelExam', lang) || 'Cancel Exam'}
                                                ...
                                                {e.status !== 'cancelled' && e.status !== 'completed' && (
                                                    <button
                                                        onClick={(event) => { event.stopPropagation(); setDeleteModal({ show: true, exam: e }); }}
                                                        title={t('delete', lang)}
                                                ...
                                                {e.status === 'completed' && (
                                                    <CheckCircle2 className="h-5 w-5 text-green-500" title={t('completed', lang)} />
                                                )}
                                            </div>
                                        </td>
                                    </motion.tr>
            ...
            {/* Exam Marks Visual — shown when a specific exam is selected */}
            <AnimatePresence>
                {applied.examid && applied.examid !== 'All' && (
                    <motion.div
                    ...
                    >
                        ...
                        {examVisual.loading ? (
                            ...
                            <>
                                ...
                                {/* Bar chart */}
                                <div style={{ height: Math.max(200, examVisual.data.length * 44) }}>
                                    <ResponsiveContainer width="100%" height="100%">
                                ...
                                {/* Score Distribution — Histogram + Bell Curve */}
                                {(() => {
                                    const bins = Array.from({ length: 10 }, (_, i) => ({ range: `${i * 10}–${i * 10 + 10}`, midpoint: i * 10 + 5, count: 0 }));
                                    examVisual.data.forEach(s => { const idx = Math.min(Math.floor(s.pct / 10), 9); bins[idx].count += 1; });
                                    const total = examVisual.data.length;
                                    const mean = total > 0 ? examVisual.data.reduce((a, b) => a + b.pct, 0) / total : 50;
                                    const variance = total > 1 ? examVisual.data.reduce((a, b) => a + Math.pow(b.pct - mean, 2), 0) / (total - 1) : 200;
                                    const std = Math.sqrt(variance) || 15;
                                    const maxCount = Math.max(...bins.map(b => b.count), 1);
                                    const bellData = bins.map(b => {
                                        const x = b.midpoint;
                                        const bell = (1 / (std * Math.sqrt(2 * Math.PI))) * Math.exp(-0.5 * Math.pow((x - mean) / std, 2));
                                        const bellScaled = parseFloat((bell * std * Math.sqrt(2 * Math.PI) * maxCount).toFixed(2));
                                        return { ...b, bell: bellScaled };
                                    });
                                    return (
                                        <div className="mt-8 pt-6 border-t border-[#f1f5f9]">
                                            <h3 className="text-sm font-bold text-[#0f172a] mb-1">{t('studentScoresDistribution', lang)}</h3>
                                            <p className="text-xs text-[#94a3b8] mb-4">{t('scoreFrequencyDesc', lang)}</p>
                                            <div style={{ height: 280 }}>
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <ComposedChart data={bellData} margin={{ top: 10, right: 30, left: 0, bottom: 30 }} barCategoryGap="10%">
                                                        <defs>
                                                        ...
                                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                                                        <XAxis dataKey="range" label={{ value: t('percentage', lang), position: 'insideBottom', offset: -18, fontSize: 12, fill: '#64748b', fontWeight: 600 }} tick={{ fill: '#94a3b8', fontSize: 11 }} tickLine={false} axisLine={{ stroke: '#e2e8f0' }} />
                                                        <YAxis label={{ value: t('students', lang), angle: -90, position: 'insideLeft', offset: 10, fontSize: 12, fill: '#64748b', fontWeight: 600 }} allowDecimals={false} tick={{ fill: '#94a3b8', fontSize: 11 }} tickLine={false} axisLine={false} width={50} />
                                                        <Tooltip
                                                            contentStyle={{ borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: 12, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}
                                                            formatter={(value, name) => [name === 'count' ? `${value} ${t('students', lang)}` : value.toFixed(2), name === 'count' ? t('markDistributionDetail', lang) : 'Normal Curve']}
                                                        />
                                                        <Legend verticalAlign="top" align="right" iconType="circle" formatter={v => <span style={{ fontSize: 11, color: '#475569', fontWeight: 600 }}>{v === 'count' ? t('markDistributionDetail', lang) : 'Normal Curve'}</span>} />
                                                        <Bar dataKey="count" name="count" fill="url(#barGrad)" radius={[4, 4, 0, 0]} animationDuration={900} />
                                                        <Line dataKey="bell" name="bell" type="monotone" stroke="#f59e0b" strokeWidth={2.5} dot={false} animationDuration={1200} />
                                                    </ComposedChart>
                                                </ResponsiveContainer>
                                            </div>
