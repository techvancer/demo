...
const SVC = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9kcnJiYXpsZWhhbW9vZ2dzb3V2Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MjQ5MTc4MSwiZXhwIjoyMDg4MDY3NzgxfQ.JmCPHQ3-EI9rggjHYQhoCi12fWz5f37ZyXLzHXUHa9w';
const BASE = 'https://odrrbazlehamooggsouv.supabase.co/rest/v1';
...
export async function fetchGradeDistribution({
    ...
    try {
        ...
        const [ansRes, qRes] = await Promise.all([
            fetch(`${BASE}/studentanswers_tbl?${ansParams}`,
                { headers: { apikey: SVC, Authorization: `Bearer ${SVC}` } }),
            fetch(`${BASE}/questions_exams_employee_subjects_sections_tbl?${qParams}`,
                { headers: { apikey: SVC, Authorization: `Bearer ${SVC}` } }),
        ]);

        const [rows, qRows] = await Promise.all([ansRes.json(), qRes.json()]);
        ...
        // studentExamData[studentid][examid] = { earned, possible }
        ...
        let gradeRows = [];
        try {
            const gRes = await fetch(`${BASE}/Grades_tbl?select=gradeid,gradename,frommark,tomark&order=tomark.desc`, {
                headers: { apikey: SVC, Authorization: `Bearer ${SVC}` }
            });
            gradeRows = await gRes.json();
        } catch {}
