...
export default function Header({ toggleSidebar, isOpen }) {
    ...
    const schoolBranchText = [user?.schoolName, user?.branchName].filter(Boolean).join(' · ');
    ...
    return (
        <header className="fixed top-0 left-0 w-full h-16 bg-[#1e3a8a] z-50 flex items-center justify-between px-3 sm:px-4 shadow-md">
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                ...
                <div className="hidden sm:flex flex-col text-right" style={{ textAlign: isAr ? 'left' : 'right' }}>
                    <span className="text-white font-medium text-sm leading-tight">{user?.name || t('user', lang)}</span>
                    <span className="text-blue-200 text-xs">{user?.subtitle || t(user?.role?.toLowerCase() || 'employee', lang)}</span>
                </div>
                ...
                {/* Avatar */}
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 border-white/40 overflow-hidden cursor-pointer flex-shrink-0">
                    <img
