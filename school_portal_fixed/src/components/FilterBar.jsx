...
export default function FilterBar({ filters = [], onApply, onReset, requiredFields = [] }) {
    const { lang } = useLang();
    ...
    useEffect(() => {
        setDraft(prev => {
            const next = { ...prev };
            filters.forEach(f => {
                // Only sync from parent if the draft key matches the parent value
                // (i.e. parent changed it, like a Reset) — don't overwrite user selections
                ...
                // If parent explicitly reset to 'All' (e.g. after Reset button), sync it
                if (parentVal === 'All' && prev[f.key] !== 'All' && f.value === 'All') {
                    next[f.key] = 'All';
                }
            });
            return next;
        });
    }, [filters.map(f => f.value).join(','), filters.map(f => (f.options||[]).length).join(',')]); // eslint-disable-line
    ...
    const activeCount = filters.filter(f => (draft[f.key] ?? 'All') !== 'All').length;
